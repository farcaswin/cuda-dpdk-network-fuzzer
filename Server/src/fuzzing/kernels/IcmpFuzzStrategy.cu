...
__global__ void icmp_fuzz_kernel(uint8_t* data, 
                                 uint16_t* lengths, 
                                 uint32_t batch_size,
                                 IcmpFuzzStrategy::Config config) {
    ...
    eth->ether_type = swap_uint16(0x0800); // IPv4
    ...
    ip->ihl = 5;
    ip->tos = 0;
    ip->total_length = swap_uint16(sizeof(IPv4Header) + sizeof(IcmpHeader));
    ...
    // Exhaustive Mutation: 65536 threads cover all 256*256 combinations of Type and Code
    icmp->type = (uint8_t)(idx / 256);
    icmp->code = (uint8_t)(idx % 256);
    icmp->id = swap_uint16(0x1234);
    icmp->sequence = swap_uint16((uint16_t)idx);
    ...
    lengths[idx] = sizeof(EthernetHeader) + sizeof(IPv4Header) + sizeof(IcmpHeader);
}

IcmpFuzzStrategy::IcmpFuzzStrategy(const Config& config) : config_(config) {}

void IcmpFuzzStrategy::launch_kernel(uint8_t* output_buffer, 
    ...
    icmp_fuzz_kernel<<<blocks_per_grid, threads_per_block, 0, stream>>>(
        output_buffer, 
        lengths_buffer, 
        batch_size,
        config_
